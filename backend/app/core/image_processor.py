...
class ImageProcessor:
            ...
            with Image.open(image_path) as img:
                ...
                if hasattr(img, '_getexif') and img._getexif() is not None:
                    ...
                    for tag_id, value in exif.items():
                        ...
                        if tag == "DateTime":
                            try:
                                exif_data["date_taken"] = datetime.strptime(str(value), "%Y:%m:%d %H:%M:%S")
                            ...
                            if hasattr(value, 'num') and hasattr(value, 'den'):
                                if value.num == 1:
                                    ...
                                    exif_data["shutter_speed"] = f"{value.num}/{value.den}"
    ...
    async def process_image(
        ...
        await asyncio.to_thread(self._process_image_sync, original_path, webp_path, thumbnail_path)
    ...
    def _process_image_sync(self, original_path: Path, webp_path: Path, thumbnail_path: Path):
        ...
        try:
            with Image.open(original_path) as img:
                ...
                # Create thumbnail
                thumbnail = img.copy()
                thumbnail.thumbnail((settings.thumbnail_size, settings.thumbnail_size), Image.Resampling.LANCZOS)
                thumbnail.save(
                    thumbnail_path,
                    format='WEBP',
                    quality=80,
