...
@pytest.mark.api
class TestProfilePicturesAPI:
    async def _create_temp_image(self, width: int, height: int) -> str:
    ...
    async def test_upload_square_image_succeeds(
        ...
        temp_file_path = await self._create_temp_image(400, 400)
        try:
    ...
    async def test_upload_non_square_image_rejected(
        ...
        temp_file_path = await self._create_temp_image(800, 300)
        try:
            with open(temp_file_path, "rb") as img_file:
    ...
    async def test_list_requires_admin(self, async_client: AsyncClient):
        """GET /api/profile-pictures is admin-only."""
        response = await async_client.get("/api/profile-pictures")
        assert response.status_code in [401, 403]

    async def test_activate_and_get_active(
        self, async_client: AsyncClient, admin_token: str
    ):
        ...
        # Upload first (square)
        path1 = await self._create_temp_image(300, 300)
        # Upload second (square)
        path2 = await self._create_temp_image(400, 400)
