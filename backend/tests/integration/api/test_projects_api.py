...
class TestProjectsAPI:
    ...
    async def test_get_projects_returns_all_projects(
        ...
        projects = await ProjectFactory.create_batch_async(test_session, 5)
    ...
    async def test_get_projects_with_technology_filter(
        ...
        react_projects = await ProjectFactory.create_batch_async(
            test_session, 3, technologies=["React", "TypeScript", "Node.js"]
        )
        python_projects = await ProjectFactory.create_batch_async(
    ...
    async def test_create_project_without_auth_returns_401(
        self, async_client: AsyncClient
    ):
    ...
    async def test_update_project_modifies_project(
        ...
        assert set(updated_project["technologies"]) == {"Python", "FastAPI", "PostgreSQL"}
...
@pytest.mark.integration
@pytest.mark.api
class TestProjectsAPIFeatures:
    """Test advanced Projects API features."""
    ...
    async def test_get_featured_projects(
        ...
        featured_projects = await ProjectFactory.create_batch_async(
            test_session, 3, is_featured=True
        )
        regular_projects = await ProjectFactory.create_batch_async(
            test_session, 2, is_featured=False
        )
    ...
    async def test_get_projects_by_status(
        ...
        completed_projects = await ProjectFactory.create_batch_async(
            test_session, 4, status="completed"
        )
        in_progress_projects = await ProjectFactory.create_batch_async(
    ...
    async def test_project_technology_statistics(
        ...
        if response.status_code == 200:  # Only test if endpoint exists
            ...
            python_count = next(
                (item["count"] for item in data if item["technology"] == "Python"), 
                0
            )
            ...
            fastapi_count = next(
                (item["count"] for item in data if item["technology"] == "FastAPI"), 
                0
            )
...
class TestProjectsAPIPerformance:
    """Test Projects API performance characteristics."""
