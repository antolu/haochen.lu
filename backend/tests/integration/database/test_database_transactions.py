...
class TestPhotoDeletionCascade:
    ...
    async def test_bulk_photo_deletion_maintains_consistency(
        ...
        for i in range(10):
            photo = await PhotoFactory.create_async(
                test_session, title=f"Bulk Test Photo {i}"
            )
            photos.append(photo)
...
class TestConcurrentModifications:
    ...
    async def test_optimistic_locking_prevents_lost_updates(
        ...
        await asyncio.gather(concurrent_update(1), concurrent_update(5))
    ...
    @pytest.mark.slow
    async def test_high_concurrency_stress_test(self, test_session: AsyncSession):
        """Stress test with high concurrency operations."""
        # Create initial data
        photos = []
        for i in range(20):
            photo = await PhotoFactory.create_async(
                test_session, title=f"Stress Test Photo {i}", view_count=0
            )
            photos.append(photo)
...
class TestTransactionIntegrity:
    ...
    async def test_transaction_atomicity_on_failure(self, test_session: AsyncSession):
    ...
    async def test_transaction_consistency_constraints(
        ...
        photo1 = await PhotoFactory.create_async(
            test_session, filename="unique_test.jpg"
        )

        # Try to create another photo with same filename (if unique constraint exists)
        with pytest.raises(IntegrityError):
            await PhotoFactory.create_async(test_session, filename="unique_test.jpg")

        await test_session.rollback()

        # Verify original photo still exists
        result = await test_session.execute(select(Photo).where(Photo.id == photo1.id))
        assert result.scalar_one_or_none() is not None

    async def test_transaction_isolation_levels(self, test_session: AsyncSession):
    ...
    async def test_transaction_durability_after_commit(
        ...
        photo = await PhotoFactory.create_async(test_session, title="Durability Test")
    ...
    async def test_nested_transaction_behavior(self, test_session: AsyncSession):
        """Test nested transaction (savepoint) behavior."""
        initial_count = await self._get_photo_count(test_session)

        # Main transaction
        await PhotoFactory.create_async(test_session, title="Main Transaction Photo")

        # Nested transaction (savepoint)
        async with test_session.begin_nested() as savepoint:
            await PhotoFactory.create_async(
                test_session, title="Nested Transaction Photo"
            )
...
class TestDataConsistencyChecks:
    ...
    async def test_referential_integrity_constraints(self, test_session: AsyncSession):
    ...
    async def test_business_logic_constraints(self, test_session: AsyncSession):
        """Test business logic constraints are enforced."""
        # Test that certain combinations of data are valid
        photo = await PhotoFactory.create_async(
            test_session, featured=True, view_count=0
        )
