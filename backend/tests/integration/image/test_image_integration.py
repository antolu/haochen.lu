...
class TestImageProcessingIntegration:
    """Test image processing integration with storage and database."""
    ...
    async def test_upload_image_creates_photo_record(
        self, async_client: AsyncClient, admin_token: str, sample_image_file: str, test_session: AsyncSession
    ):
    ...
    async def test_image_processing_creates_multiple_formats(
        self, async_client: AsyncClient, admin_token: str, sample_image_file: str
    ):
        ...
        assert "original_url" in photo_data
        assert "webp_url" in photo_data  # WebP version
        assert "thumbnail_url" in photo_data  # Thumbnail
    ...
    async def test_image_optimization_reduces_file_size(
        ...
        try:
            ...
            # WebP version should be smaller
            if "webp_url" in photo_data:
                assert photo_data["webp_url"] is not None
    ...
    async def test_image_update_preserves_files(
        ...
        try:
            ...
            original_urls = {
                key: photo_data[key] 
                for key in ["original_url", "thumbnail_url", "webp_url"] 
            ...
            # File URLs should remain the same
            for key, original_url in original_urls.items():
                if key in updated_data:
                    assert updated_data[key] == original_url
        
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
    
    @patch('app.services.storage.StorageService.upload_file')
    async def test_storage_service_integration(
        self, mock_upload, async_client: AsyncClient, admin_token: str
    ):
        """Test integration with storage service."""
        # Mock storage service
        mock_upload.return_value = {
            "url": "https://storage.example.com/photo123.jpg",
            "size": 12345
        }
        ...
        try:
            with open(temp_file_path, 'rb') as img_file:
                files = {"file": ("storage_test.jpg", img_file, "image/jpeg")}
                data = {"title": "Storage Test"}
            ...
            # Should succeed even with mocked storage
            assert response.status_code == 201
            
            # Storage service should have been called
...
class TestImageProcessingPerformance:
    """Test image processing performance and resource usage."""
    
    async def test_large_image_processing_performance(self, admin_token: str, async_client: AsyncClient):
    ...
    async def test_memory_usage_during_batch_processing(self, admin_token: str, async_client: AsyncClient):
