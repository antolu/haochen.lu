...
class TestImageProcessing:
    ...
    async def test_extract_exif_from_image_without_metadata(
        ...
        try:
            ...
            assert exif_data.get('gps_latitude') is None or 'gps_latitude' not in exif_data
    ...
    async def test_convert_to_webp_maintains_quality(
        ...
        try:
            webp_bytes = await image_processor.convert_to_webp(temp_file_path, quality=85)
    ...
    async def test_resize_image_maintains_aspect_ratio(
        ...
        try:
            ...
            resized_bytes = await image_processor.resize_image(temp_file_path, max_width=400)
    ...
    async def test_process_invalid_image_file_raises_error(
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg', mode='w', delete=False) as temp_file:
    ...
    async def test_concurrent_image_processing_safe(
        ...
        try:
            ...
            tasks = [
                image_processor.extract_exif(temp_file) 
    ...
    async def test_exif_orientation_handling(
        self, image_processor: ImageProcessor
    ):
...
class TestImageValidation:
    ...
    async def test_validate_image_size_within_limits(
        ...
        result = await image_processor.validate_image_size(
            len(sample_jpeg_bytes), max_size=10_000_000  # 10MB
        )
    ...
    async def test_validate_image_size_exceeds_limits(
        ...
        with pytest.raises(ImageProcessingError):
            await image_processor.validate_image_size(
                large_size, max_size=10_000_000  # 10MB limit
            )
