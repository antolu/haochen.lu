...
class TestPortfolioOrganization:
    ...
    async def test_organize_photos_by_category(
        ...
        await PhotoFactory.create_batch_async(test_session, 2, category="street")
    ...
    async def test_generate_photo_recommendations(
        ...
        assert all(
            "score" in photo for photo in recommendations
    ...
    async def test_calculate_portfolio_statistics(
        ...
        await ProjectFactory.create_batch_async(test_session, 3, status="completed")
    ...
    async def test_validate_portfolio_completeness(
        ...
        validation = await portfolio_service.validate_portfolio_completeness(
            test_session
        )
...
class TestProjectManagement:
    ...
    async def test_project_status_transitions(
        ...
        project = await ProjectFactory.create_async(test_session, status="planning")
    ...
    async def test_invalid_project_status_transitions(
        ...
        project = await ProjectFactory.create_async(test_session, status="completed")
        ...
        with pytest.raises(BusinessLogicError):
            await portfolio_service.update_project_status(
                test_session,
                project.id,
                "planning",  # Can't go back to planning
            )
    ...
    async def test_featured_projects_selection(
        ...
        await ProjectFactory.create_batch_async(
            test_session, 2, status="in_progress", technologies=["HTML"], demo_url=None
        )
    ...
    async def test_project_technology_recommendations(
        ...
        python_rec = next(
            (rec for rec in recommendations if rec["technology"] == "Python"), None
        )
...
class TestPortfolioSEO:
    ...
    async def test_generate_portfolio_sitemap_data(
        ...
        await PhotoFactory.create_batch_async(test_session, 5, is_public=True)
        await ProjectFactory.create_batch_async(test_session, 3, status="completed")
