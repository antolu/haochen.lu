...
def test_create_access_token_basic(mock_settings):
...
def test_create_access_token_expiry(mock_settings):
...
def test_create_refresh_token_with_remember_me(mock_settings):
    ...
    assert token is not None
    payload = jwt.decode(token, mock_settings.session_secret_key, algorithms=["HS256"])
...
def test_create_refresh_token_without_remember_me(mock_settings):
...
def test_create_refresh_token_expiry_with_remember_me(mock_settings):
...
def test_create_refresh_token_expiry_without_remember_me(mock_settings):
...
def test_refresh_token_jti_uniqueness(mock_settings):
...
def test_verify_refresh_token_invalid_signature(mock_settings):
...
def test_verify_refresh_token_expired(mock_settings):
...
def test_verify_refresh_token_malformed(mock_settings):
...
def test_set_refresh_cookie_with_remember_me(mock_response, mock_settings):
...
def test_set_refresh_cookie_without_remember_me(mock_response, mock_settings):
...
def test_unset_refresh_cookie(mock_response, mock_settings):
...
def test_access_and_refresh_tokens_use_different_secrets(mock_settings):
    ...
    with pytest.raises((jwt.JWTError, ValueError)):
        jwt.decode(access_token, mock_settings.session_secret_key, algorithms=["HS256"])

    with pytest.raises((jwt.JWTError, ValueError)):
        jwt.decode(refresh_token, mock_settings.SECRET_KEY, algorithms=["HS256"])


def test_jti_is_secure_random(mock_settings):
...
def test_token_type_enforcement(mock_settings):
...
def test_token_expiry_enforcement(mock_settings):
    ...
    with freeze_time("2024-01-01 12:00:00") as frozen_time:
        ...
        # Test refresh token expiry with remember me
        refresh_token_long = TokenManager.create_refresh_token(data, remember_me=True)
        ...
        # Test refresh token expiry without remember me
        refresh_token_short = TokenManager.create_refresh_token(data, remember_me=False)
...
def test_empty_data_handling(mock_settings):
...
def test_large_data_handling(mock_settings):
...
def test_special_characters_in_data(mock_settings):
...
def test_none_values_in_data(mock_settings):
...
def test_cookie_settings_edge_cases(mock_response, mock_settings):
...
def test_jti_generation_uses_uuid(mock_uuid4, mock_settings):
...
def test_complete_token_lifecycle(mock_response, mock_settings):
...
def test_token_rotation_simulation(mock_settings):
