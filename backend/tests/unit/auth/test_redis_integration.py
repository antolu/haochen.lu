...
class TestRedisClient:
    ...
    async def test_disconnect(self, redis_test_client):
    ...
    async def test_is_connected_true(self, redis_test_client):
        """Test is_connected when Redis is available."""
        result = await redis_test_client.is_connected()
        assert result is True

    async def test_is_connected_false_no_redis(self, redis_unavailable_client):
        """Test is_connected when Redis is not available."""
        result = await redis_unavailable_client.is_connected()
        assert result is False

    async def test_is_connected_false_ping_fails(self, redis_test_client):
    ...
    async def test_set_success(self, redis_test_client):
    ...
    async def test_set_with_expiry(self, redis_test_client):
    ...
    async def test_set_failure_no_connection(self, redis_unavailable_client):
        """Test set operation when Redis is unavailable."""
        result = await redis_unavailable_client.set("test_key", "test_value")
        assert result is False

    async def test_setex_success(self, redis_test_client):
    ...
    async def test_get_success(self, redis_test_client):
    ...
    async def test_keys_no_matches(self, redis_test_client):
        """Test keys pattern with no matches."""
        result = await redis_test_client.keys("nonexistent:*")
        assert result == []

    async def test_keys_failure_no_connection(self, redis_unavailable_client):
        """Test keys operation when Redis is unavailable."""
        result = await redis_unavailable_client.keys("test:*")
        assert result == []

    async def test_exists_true(self, redis_test_client):
    ...
    async def test_ttl_no_expiry(self, redis_test_client):
    ...
    async def test_ttl_nonexistent_key(self, redis_test_client):
        """Test TTL for nonexistent key."""
        result = await redis_test_client.ttl("nonexistent_key")
        assert result == -2

    async def test_ttl_failure_no_connection(self, redis_unavailable_client):
...
class TestTokenManager:
    """Test TokenManager Redis operations."""

    def test_refresh_token_key_generation(self):
    ...
    async def test_store_refresh_token_success(self, redis_test_client):
    ...
    async def test_store_refresh_token_failure(self, redis_unavailable_client):
        """Test storing refresh token when Redis is unavailable."""
        with patch("app.core.redis.redis_client", redis_unavailable_client):
            result = await TokenManager.store_refresh_token("user123", "jti456", 3600)
            assert result is False

    async def test_is_refresh_token_valid_true(self, redis_test_client):
    ...
    async def test_is_refresh_token_valid_false(self, redis_test_client):
        """Test checking invalid refresh token."""
        with patch("app.core.redis.redis_client", redis_test_client):
            result = await TokenManager.is_refresh_token_valid("user123", "jti456")
            assert result is False

    async def test_is_refresh_token_valid_unavailable(self, redis_unavailable_client):
        """Test checking refresh token when Redis is unavailable."""
        with patch("app.core.redis.redis_client", redis_unavailable_client):
            result = await TokenManager.is_refresh_token_valid("user123", "jti456")
            assert result is False

    async def test_revoke_refresh_token_success(self, redis_test_client):
    ...
    async def test_revoke_refresh_token_nonexistent(self, redis_test_client):
        """Test revoking nonexistent refresh token."""
        with patch("app.core.redis.redis_client", redis_test_client):
            result = await TokenManager.revoke_refresh_token("user123", "jti456")
            assert result is False

    async def test_revoke_all_user_tokens_success(self, redis_test_client):
    ...
    async def test_revoke_all_user_tokens_no_tokens(self, redis_test_client):
        """Test revoking all tokens when user has no tokens."""
        with patch("app.core.redis.redis_client", redis_test_client):
            result = await TokenManager.revoke_all_user_tokens("user123")
            assert result == 0

    async def test_get_user_token_count(self, redis_test_client):
...
class TestSessionManager:
    """Test SessionManager Redis operations."""
    ...
    async def test_store_session_success(self, redis_test_client):
    ...
    async def test_store_session_failure(self, redis_unavailable_client):
        """Test storing session when Redis is unavailable."""
        with patch("app.core.redis.redis_client", redis_unavailable_client):
            result = await SessionManager.store_session("session123", "user456", 1800)
            assert result is False

    async def test_get_session_user_success(self, redis_test_client):
    ...
    async def test_get_session_user_nonexistent(self, redis_test_client):
        """Test getting user from nonexistent session."""
        with patch("app.core.redis.redis_client", redis_test_client):
            user_id = await SessionManager.get_session_user("session123")
            assert user_id is None

    async def test_get_session_user_unavailable(self, redis_unavailable_client):
        """Test getting user when Redis is unavailable."""
        with patch("app.core.redis.redis_client", redis_unavailable_client):
            user_id = await SessionManager.get_session_user("session123")
            assert user_id is None

    async def test_revoke_session_success(self, redis_test_client):
    ...
    async def test_revoke_session_nonexistent(self, redis_test_client):
        """Test revoking nonexistent session."""
        with patch("app.core.redis.redis_client", redis_test_client):
            result = await SessionManager.revoke_session("session123")
            assert result is False

    async def test_extend_session_success(self, redis_test_client):
    ...
    async def test_extend_session_nonexistent(self, redis_test_client):
...
class TestErrorHandling:
    ...
    async def test_redis_operation_exception_handling(self, redis_test_client):
    ...
    async def test_connection_lost_during_operation(self, redis_test_client):
        """Test handling connection loss during operation."""
        # Simulate connection being lost
        redis_test_client._redis.ping = AsyncMock(
    ...
    async def test_redis_timeout_handling(self, redis_test_client):
...
class TestConcurrency:
    ...
    async def test_concurrent_token_operations(self, redis_test_client):
    ...
    async def test_concurrent_user_token_revocation(self, redis_test_client):
...
class TestEdgeCases:
    ...
    async def test_very_long_keys(self, redis_test_client):
    ...
    async def test_very_long_values(self, redis_test_client):
    ...
    async def test_special_characters_in_keys_and_values(self, redis_test_client):
    ...
    async def test_zero_expiry_time(self, redis_test_client):
    ...
    async def test_negative_expiry_time(self, redis_test_client):
    ...
    async def test_empty_string_operations(self, redis_test_client):
