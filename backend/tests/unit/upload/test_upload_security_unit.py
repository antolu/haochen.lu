...
@pytest.fixture
def image_processor(temp_upload_dir, temp_compressed_dir):
    """Create an image processor for testing."""
    return ImageProcessor(str(temp_upload_dir), str(temp_compressed_dir))


# Helper methods (moved from TestFileTypeValidation)
def _is_valid_image_type(filename: str, mime_type: str) -> bool:
...
def _simple_sanitize_filename(filename: str) -> str:
...
def _is_safe_image_file(filepath: str) -> bool:
...
def _sanitize_metadata(metadata: str) -> str:
...
def test_accept_valid_image_formats(image_processor):
    ...
    for filename, mime_type in valid_formats:
        ...
        assert _is_valid_image_type(filename, mime_type)


def test_reject_dangerous_file_types(image_processor):
    ...
    for filename, mime_type in dangerous_files:
        assert not _is_valid_image_type(filename, mime_type)


def test_file_extension_and_mime_type_validation(image_processor):
    """Test that both extension and MIME type are validated."""
    # File with image extension but wrong MIME type
    assert not _is_valid_image_type("image.jpg", "application/javascript")

    # File with executable extension but image MIME type
    assert not _is_valid_image_type("script.exe", "image/jpeg")

    # Consistent image file should pass
    assert _is_valid_image_type("photo.jpg", "image/jpeg")


def test_double_extension_attack_prevention(image_processor):
    ...
    for filename in malicious_files:
        ...
        assert not _is_valid_filename(filename)


def test_null_byte_injection_prevention(image_processor):
    ...
    for filename in malicious_filenames:
        ...
        sanitized = _simple_sanitize_filename(filename)
        assert "\x00" not in sanitized
        assert "%00" not in sanitized


def test_magic_number_file_header_validation(image_processor):
    ...
    for data in fake_image_data.values():
        with tempfile.NamedTemporaryFile(suffix=".jpg") as temp_file:
            ...
            assert not _is_valid_image_file(temp_file.name)


def test_real_image_magic_number_validation(sample_image_data):
    ...
    with tempfile.NamedTemporaryFile(suffix=".jpg") as temp_file:
        ...
        assert _is_valid_image_file(temp_file.name)


def test_polyglot_file_detection(image_processor):
    ...
    for pattern in suspicious_patterns:
        ...
        with tempfile.NamedTemporaryFile(suffix=".jpg") as temp_file:
            ...
            is_safe = _is_safe_image_file(temp_file.name)
            assert is_safe is False or is_safe is None  # Depends on implementation


def test_embedded_metadata_security(image_processor):
...
# Helper methods (moved from TestFileSizeValidation)
def _is_valid_file_size(file_size: int, max_size: int) -> bool:
...
def test_reject_oversized_files(image_processor):
    ...
    assert not _is_valid_file_size(oversized_file_size, max_size)


def test_accept_files_within_size_limit(image_processor):
    ...
    for file_size in valid_file_sizes:
        assert _is_valid_file_size(file_size, max_size)


def test_memory_exhaustion_prevention(image_processor):
    ...
    with tempfile.NamedTemporaryFile() as temp_file:
        ...
        try:
            ...
            _process_file_streaming(temp_file.name)
...
def test_streaming_upload_for_large_files(image_processor):
...
def test_disk_space_checking_before_upload(image_processor):
    ...
    with patch("shutil.disk_usage") as mock_disk_usage:
        ...
        has_space = _has_sufficient_disk_space(required_space)
...
def test_remove_directory_traversal_attempts():
    ...
    for filename in malicious_filenames:
        sanitized = _complex_sanitize_filename(filename)
...
def test_sanitize_special_characters():
    ...
    for original in special_chars:
        sanitized = _complex_sanitize_filename(original)
...
def test_generate_unique_filenames_prevent_overwrites():
...
def test_extremely_long_filename_handling():
    """Test handling of extremely long filenames."""
    # Create a very long filename
    long_name = "a" * 300 + ".jpg"

    sanitized = _complex_sanitize_filename(long_name)
...
def test_reserved_filename_handling():
    ...
    for reserved_name in reserved_names:
        sanitized = _complex_sanitize_filename(reserved_name)
