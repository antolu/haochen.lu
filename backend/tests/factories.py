...
class PhotoFactory(AsyncSQLAlchemyModelFactory):
    ...
    camera_make = Faker(
        "random_element", elements=("Canon", "Nikon", "Sony", "Fujifilm")
    )
    camera_model = Faker(
        "random_element", elements=("EOS R5", "D850", "A7R IV", "X-T4")
    )
    lens = Faker(
        "random_element", elements=("24-70mm f/2.8", "85mm f/1.4", "16-35mm f/2.8")
    )
    iso = Faker("random_element", elements=(100, 200, 400, 800, 1600))
    aperture = Faker("random_element", elements=(1.4, 1.8, 2.8, 4.0, 5.6))
    shutter_speed = Faker(
        "random_element", elements=("1/60", "1/125", "1/250", "1/500")
    )
...
class BlogPostFactory(AsyncSQLAlchemyModelFactory):
    ...
    featured_image = LazyAttribute(
        lambda obj: f"https://images.unsplash.com/photo-{obj.id}"
    )

    tags = Faker("words", nb=4)
    category = Faker(
        "random_element", elements=("photography", "technology", "travel", "tutorial")
    )
...
class TestDataSet:
    ...
    async def create_complete_portfolio(cls, session: AsyncSession) -> dict[str, Any]:
        ...
        return {
            "admin": admin,
            "photos": {"featured": featured_photos, "regular": regular_photos},
            "projects": {"featured": featured_projects, "regular": regular_projects},
            "blog_posts": {"published": published_posts, "draft": draft_posts},
            "sub_apps": {"external": external_apps, "internal": internal_apps},
        }

    @classmethod
    async def create_performance_dataset(
        cls, session: AsyncSession, size: int = 1000
    ) -> dict[str, Any]:
        ...
        return {"photos": photos, "projects": projects, "blog_posts": blog_posts}
