...
class TestAPIPerformance:
    ...
    async def test_photos_list_performance_under_load(
        ...
        ratio = (
            performance_metrics["limit_100"]["avg"]
            / performance_metrics["limit_10"]["avg"]
        )
    ...
    async def test_database_query_performance(
        ...
        for query in complex_queries:
            ...
            avg_time = sum(times) / len(times)
            assert avg_time < 0.8, (
                f"Complex query too slow: {avg_time:.3f}s for {query}"
            )
...
class TestMemoryPerformance:
    ...
    async def test_memory_leak_detection_during_requests(
        ...
        assert memory_trend < 50, (
            f"Potential memory leak: {memory_trend:.2f}MB increase"
        )
...
class TestImageProcessingPerformance:
        ...
        for width, height, size_name in image_sizes:
            ...
            try:
                ...
                for i in range(3):  # 3 uploads per size
                    ...
                    with open(temp_file_path, "rb") as img_file:
                        files = {
                            "file": (f"{size_name}_{i}.jpg", img_file, "image/jpeg")
                        }
    ...
    async def test_concurrent_image_processing_performance(
        ...
        try:
            ...
            avg_processing_time = sum(r["processing_time"] for r in results) / len(
                results
            )
