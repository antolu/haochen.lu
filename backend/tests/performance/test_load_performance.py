...
class TestAPIPerformance:
    ...
    async def test_photos_list_performance_under_load(
        ...
        for limit in [10, 50, 100]:
            ...
            assert avg_time < 1.0, f"Average response time {avg_time:.3f}s too slow for limit {limit}"
            assert max_time < 2.0, f"Max response time {max_time:.3f}s too slow for limit {limit}"
        
        # Larger page sizes should not be exponentially slower
        ratio = performance_metrics["limit_100"]["avg"] / performance_metrics["limit_10"]["avg"]
    ...
    async def test_concurrent_api_requests_performance(
        ...
        assert len(successful_requests) >= 25, f"Too many failed requests: {len(failed_requests)}"
        
        avg_response_time = sum(r["response_time"] for r in successful_requests) / len(successful_requests)
        max_response_time = max(r["response_time"] for r in successful_requests)
        
        # Performance expectations
        assert avg_response_time < 2.0, f"Average response time {avg_response_time:.3f}s too slow"
    ...
    async def test_memory_usage_during_batch_operations(
        ...
        try:
            for i in range(10):
                ...
                temp_file = tempfile.NamedTemporaryFile(suffix=f'_mem_test_{i}.jpg', delete=False)
            ...
            assert memory_increase < 200, f"Memory increase {memory_increase:.2f}MB too high"
            assert max_memory_increase < 300, f"Peak memory increase {max_memory_increase:.2f}MB too high"
...
class TestImageProcessingPerformance:
    """Test image processing performance characteristics."""
    ...
    async def test_image_upload_processing_performance(
        ...
        for width, height, size_name in image_sizes:
            ...
            with tempfile.NamedTemporaryFile(suffix=f'_{size_name}.jpg', delete=False) as temp_file:
        ...
        assert scaling_ratio < 10.0, f"Image processing scaling ratio {scaling_ratio:.2f} too high"
    ...
    async def test_concurrent_image_processing_performance(
        ...
        try:
            for i in range(5):
                ...
                temp_file = tempfile.NamedTemporaryFile(suffix=f'_concurrent_{i}.jpg', delete=False)
            ...
            assert total_time < 25.0, f"Total concurrent processing time {total_time:.3f}s too slow"
            assert avg_processing_time < 15.0, f"Average processing time {avg_processing_time:.3f}s too slow"
            assert max_processing_time < 20.0, f"Max processing time {max_processing_time:.3f}s too slow"
...
class TestDatabasePerformance:
    ...
    async def test_large_dataset_query_performance(
        ...
        for batch_start in range(0, total_records, batch_size):
            ...
            for i in range(batch_start, min(batch_start + batch_size, total_records)):
                batch_photos.append(await PhotoFactory.create_async(
                    test_session,
                    category="landscape" if i % 4 == 0 else "portrait" if i % 4 == 1 else "street",
        ...
        query_tests = [
            ...
            ("/api/photos?is_public=true&sort=created_at&order=desc&limit=40", 0.8),  # Complex query
        ...
        for query, max_time in query_tests:
            ...
            assert avg_time < max_time, f"Query too slow: {avg_time:.3f}s > {max_time}s for {query}"
            assert max_query_time < max_time * 2, f"Worst case too slow: {max_query_time:.3f}s for {query}"
    ...
    async def test_database_connection_pool_performance(
        ...
        async def make_db_intensive_request():
            ...
            response = await async_client.get("/api/photos?sort=created_at&order=desc&limit=20")
            return response.status_code == 200
        ...
        assert successful_requests >= 20, f"Connection pool issues: only {successful_requests}/{concurrent_requests} succeeded"
...
class TestPerformanceBenchmarks:
    """Performance benchmarking tests for establishing baselines."""
    ...
    async def test_throughput_benchmarks(
        ...
        assert requests_per_second >= 10.0, f"Throughput too low: {requests_per_second:.2f} req/s"
