...
describe('Target Size Calculation', () => {
  test('calculates appropriate target sizes for different use cases', () => {
    ...
    const standardThumbnail = calculateTargetSize(ImageUseCase.THUMBNAIL, standardContext);
    const retinaThumbnail = calculateTargetSize(ImageUseCase.THUMBNAIL, retinaContext);
    expect(retinaThumbnail).toBeGreaterThan(standardThumbnail);

    // Gallery images should scale with DPI
    const standardGallery = calculateTargetSize(ImageUseCase.GALLERY, standardContext);
    const retinaGallery = calculateTargetSize(ImageUseCase.GALLERY, retinaContext);
    expect(retinaGallery).toBe(standardGallery * 2);

    // Lightbox should request high quality for retina
    const retinaLightbox = calculateTargetSize(ImageUseCase.LIGHTBOX, retinaContext);
  ...
  test('respects container size when provided', () => {
    ...
    const targetSize = calculateTargetSize(ImageUseCase.GALLERY, context, containerSize);
...
describe('Complete Image Selection', () => {
  ...
  test('selects appropriate variants for different use cases on retina display', () => {
    ...
    const thumbnailSelection = selectOptimalImage(mockPhoto, ImageUseCase.THUMBNAIL);
    expect(thumbnailSelection.selectedVariant).toBe('small');
    expect(thumbnailSelection.url).toBe('/api/photos/test-photo-123/file/small');

    // Gallery usage - should prefer medium/large for retina
    const gallerySelection = selectOptimalImage(mockPhoto, ImageUseCase.GALLERY);
    expect(['medium', 'large'].includes(gallerySelection.selectedVariant)).toBe(true);

    // Lightbox usage - should prefer xlarge for retina
    const lightboxSelection = selectOptimalImage(mockPhoto, ImageUseCase.LIGHTBOX);
    expect(['large', 'xlarge'].includes(lightboxSelection.selectedVariant)).toBe(true);
  });
...
describe('Standard Display Behavior', () => {
  ...
  test('selects smaller variants for standard displays', () => {
    ...
    const gallerySelection = selectOptimalImage(mockPhoto, ImageUseCase.GALLERY);
    expect(['small', 'medium'].includes(gallerySelection.selectedVariant)).toBe(true);

    // Lightbox on standard display should still get good quality but not necessarily xlarge
    const lightboxSelection = selectOptimalImage(mockPhoto, ImageUseCase.LIGHTBOX);
    expect(['medium', 'large', 'xlarge'].includes(lightboxSelection.selectedVariant)).toBe(true);
  });
});
...
describe('Mobile Device Behavior', () => {
  ...
  test('provides appropriate quality for mobile ultra-high DPI', () => {
    ...
    expect(['medium', 'large', 'xlarge'].includes(gallerySelection.selectedVariant)).toBe(true);
  });
});
