...
describe('LocationInput', () => {
  ...
  describe('Form Validation and Error Handling', () => {
    ...
    it('validates coordinate ranges', async () => {
      ...
      render(<LocationInput onLocationChange={onLocationChange} longitude={-122.4194} />);
  ...
  describe('UI/UX Features', () => {
    ...
    it('applies custom className', () => {
      render(<LocationInput {...defaultProps} className="custom-location-input" />);

      const container = screen.getByLabelText('Location Name').closest('.custom-location-input');
  ...
  describe('Integration with External APIs', () => {
    ...
    it('handles network timeout gracefully', async () => {
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith('Error reverse geocoding:', expect.any(Error));
      });
