...
vi.mock("leaflet", () => {
  ...
  const mockDivIcon = vi.fn(
    (config: { className?: string; html?: string }) => ({
      options: config,
...
vi.mock("react-leaflet", () => {
  return {
    ...
    TileLayer: (props: any) => (
      <div data-testid="photo-tile-layer" {...props} />
    ),
    Marker: ({
      children,
      eventHandlers,
...
describe("PhotoMap", () => {
  ...
  it("shows empty state when no geotagged photos", () => {
    const photosWithoutLocation = mockPhotos.filter(
      (photo) => !photo.location_lat,
    );
    render(<PhotoMap photos={photosWithoutLocation} />);

    expect(screen.getByText("No geotagged photos")).toBeInTheDocument();
    expect(
      screen.getByText(/Upload photos with GPS coordinates/),
    ).toBeInTheDocument();
  });
  ...
  it("displays photo count summary", () => {
    ...
    expect(
      screen.getByText("Showing 2 geotagged photos of 3 total"),
    ).toBeInTheDocument();
  });
  ...
  it("handles single geotagged photo", () => {
    ...
    expect(
      screen.getByText("Showing 1 geotagged photo of 1 total"),
    ).toBeInTheDocument();
  });
  ...
  it("displays photo information in popup", () => {
    ...
    expect(
      screen.getByText("📍 Golden Gate Bridge, San Francisco"),
    ).toBeInTheDocument();
  ...
  describe("Map Bounds and Centering", () => {
    ...
    it("uses default center when no photos", () => {
      ...
      expect(
        screen.queryByTestId("photo-map-container"),
  ...
  describe("Popup Content", () => {
    it("shows all available metadata in popup", () => {
      ...
      expect(screen.getByText("Golden Gate Bridge")).toBeInTheDocument();
      expect(
        screen.getByText("📍 Golden Gate Bridge, San Francisco, CA"),
      ).toBeInTheDocument();
      expect(screen.getByText("📷 Canon EOS R5")).toBeInTheDocument();
  ...
  describe("Empty State", () => {
    ...
    it("applies correct styling to empty state", () => {
      ...
      const emptyStateContainer = screen.getByText("No geotagged photos")
  ...
  describe("Performance", () => {
    it("handles large number of photos efficiently", () => {
      ...
      expect(screen.getByTestId("photo-map-container")).toBeInTheDocument();
      expect(
        screen.getByText("Showing 100 geotagged photos of 100 total"),
      ).toBeInTheDocument();
    });
    ...
    it("filters geotagged photos efficiently", () => {
      ...
      expect(
        screen.getByText("Showing 2 geotagged photos of 4 total"),
      ).toBeInTheDocument();
