...
describe('ProfilePictureManager', () => {
  it('shows upload modal and calls upload API', async () => {
    ...
    const activeSpy = vi.spyOn(api.profilePictures, 'getActive').mockResolvedValue({
      profile_picture: null,
    });
    const uploadSpy = vi.spyOn(api.profilePictures, 'upload').mockResolvedValue({
      id: 'pp-1',
      title: 'P1',
      filename: 'p1.jpg',
    ...
    const { getByText, findByText } = renderWithProviders(<ProfilePictureManager />);
    ...
    await uploadSpy({ file: new File(['xx'], 'x.jpg', { type: 'image/jpeg' }) });
