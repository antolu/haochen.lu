...
test.describe("Map Interaction and Location Editing", () => {
  ...
  test.describe("Photo Edit Form Location Tab", () => {
    test("allows editing location through map picker", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="location-name-input"]'),
      ).toHaveValue("Golden Gate Bridge, San Francisco, CA");
      await expect(page.locator('[data-testid="latitude-input"]')).toHaveValue(
        "37.8199",
      );
      await expect(page.locator('[data-testid="longitude-input"]')).toHaveValue(
        "-122.4783",
      );
      ...
      // Wait for map to load
      await expect(
        page.locator('[data-testid="map-picker"] .leaflet-container'),
      ).toBeVisible();

      // Click on a different location on the map
      const mapContainer = page.locator(
        '[data-testid="map-picker"] .leaflet-container',
      );
      const mapBounds = await mapContainer.boundingBox();
      ...
      await expect(
        page.locator('[data-testid="location-name-input"]'),
      ).toHaveValue("Reverse Geocoded Location");
    ...
    test("validates coordinate ranges", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="validation-error"]'),
      ).toContainText("Invalid coordinates");
    });
  });
  ...
  test.describe("Location Search and Selection", () => {
    test("searches and selects locations from dropdown", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="location-name-input"]'),
      ).toHaveValue("San Francisco, California, United States");
      await expect(page.locator('[data-testid="latitude-input"]')).toHaveValue(
        "37.7749",
      );
      await expect(page.locator('[data-testid="longitude-input"]')).toHaveValue(
  ...
  test.describe("Photo Metadata Overlay with Location", () => {
    test("shows location information in metadata overlay", async ({ page }) => {
      ...
      const metadataToggle = page.locator(
        '[data-testid="show-metadata-toggle"]',
      );
      ...
      const metadataOverlay = firstPhotoCard.locator(
  ...
  test.describe("Error Handling and Edge Cases", () => {
    ...
    test("handles geolocation permission denied", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="latitude-input"]'),
      ).toBeEditable();
      await expect(
        page.locator('[data-testid="longitude-input"]'),
      ).toBeEditable();
    });
  });

  test.describe("Accessibility and Keyboard Navigation", () => {
    test("supports keyboard navigation in location search", async ({
      page,
    }) => {
      await page.goto("/admin/photos");
      await page.click('[data-testid="edit-photo-1"]');
    ...
    test("provides proper ARIA labels and roles", async ({ page }) => {
      ...
      // Check form labels
      await expect(page.locator('label[for="location-name"]')).toContainText(
        "Location Name",
      );
      await expect(page.locator('label[for="latitude"]')).toContainText(
        "Latitude",
      );
      await expect(page.locator('label[for="longitude"]')).toContainText(
        "Longitude",
      );

      // Check input accessibility attributes
      const latInput = page.locator('[data-testid="latitude-input"]');
      await expect(latInput).toHaveAttribute(
        "aria-label",
        "Latitude coordinate",
      );
