...
test.describe('Session Persistence E2E Tests', () => {
  ...
  test.describe('Login Flow', () => {
    test('should login successfully with remember me', async ({ page }) => {
      ...
      await expect(page.locator('text=Dashboard') || page.locator('text=Admin')).toBeVisible();
    });
  ...
  test.describe('Session Persistence', () => {
    test('should persist session with remember me across browser restart', async ({ browser }) => {
    ...
    test('should not persist session without remember me across browser restart', async ({
      ...
      await expect(newPage.locator('text=Sign in to Admin Panel')).toBeVisible();
  ...
  test.describe('Multi-Tab Behavior', () => {
    test('should maintain session across multiple tabs', async ({ context }) => {
      ...
      await expect(page1.locator('text=Dashboard') || page1.locator('text=Admin')).toBeVisible();
      await expect(page2.locator('text=Dashboard') || page2.locator('text=Admin')).toBeVisible();
    });

    test('should logout from all tabs when using logout everywhere', async ({ context }) => {
      ...
      const logoutEverywhereButton = page1.locator('text=Logout from all devices');
      if (await logoutEverywhereButton.isVisible()) {
        ...
        await expect(page1.locator('text=Sign in to Admin Panel')).toBeVisible();
        await expect(page2.locator('text=Sign in to Admin Panel')).toBeVisible();
      }
    });
  });
  ...
  test.describe('Token Refresh', () => {
    ...
    test('should redirect to login if token refresh fails', async ({ page }) => {
      await login(page, true);
  ...
  test.describe('Performance and UX', () => {
    ...
    test('should maintain UI responsiveness during authentication', async ({ page }) => {
      await page.goto('/login');
