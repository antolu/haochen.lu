...
describe("Session Flow Integration Tests", () => {
  ...
  describe("Automatic Token Refresh", () => {
    ...
    it("should logout and redirect if refresh fails", async () => {
      ...
      mockAxios
        .onGet("/protected-resource")
        .reply(401, { detail: "Token expired" });
      ...
      // Restore
      const spy = authStore.refreshToken as unknown as {
        mockRestore?: () => void;
      };
  ...
  describe("Session Persistence", () => {
    it("should restore session on checkAuth with valid token", async () => {
      ...
      expect(
        authStore.user === null ||
          authStore.user?.username === mockUser.username,
      ).toBe(true);
    });
    ...
    it("should refresh token during checkAuth if expired", async () => {
      ...
      const didRefresh = mockAxios.history.post.some(
  ...
  describe("Logout Functionality", () => {
    ...
    it("should logout everywhere successfully", async () => {
      ...
      const logoutEverywhereButton = screen.getByText(
        /logout from all devices/i,
      );
      ...
      await waitFor(() => {
        expect(
          mockAxios.history.post.some(
            (req) => req.url === "/auth/revoke-all-sessions",
          ),
        ).toBe(true);
        expect(useAuthStore.getState().isAuthenticated).toBe(false);
      });
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle network errors during login", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /network error|invalid credentials|invalid username|access denied/i,
          ),
        ).toBeInTheDocument();
      });
